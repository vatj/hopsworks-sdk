...
impl DatasetApi {
    ...
    pub fn download(
        ...
        let file_size = self._get(path)?.attributes.size.parse::<usize>().unwrap();
        let response = _client._send_request("GET", &path_params, Some(&query_params), true)?;
    ...
    fn _get_flow_base_params(
        ...
        base_params.insert("templateId", "-1".to_string());
        base_params.insert("flowChunkSize", chunk_size.to_string());
        base_params.insert("flowTotalSize", size.to_string());
        base_params.insert("flowIdentifier", format!("{}_{}", size, file_name));
        base_params.insert("flowFilename", file_name.to_string());
        base_params.insert("flowRelativePath", file_name.to_string());
        base_params.insert("flowTotalChunks", num_chunks.to_string());
        base_params
    }
