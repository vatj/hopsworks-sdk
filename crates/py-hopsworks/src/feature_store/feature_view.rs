...
use pyo3::{prelude::*, types::{PyDict, PyString, PyInt, PyBool}};
...
impl PyFeatureView {
    ...
    pub fn get_feature_vector(&self, entry: &Bound<PyAny>, _passed_values: Option<&Bound<PyAny>>, _rest_read_options: Option<&Bound<PyAny>>) -> PyResult<()> {
        ...
        let multithreaded = *crate::MULTITHREADED.get().unwrap();
        let sfv = hopsworks_api::online_store::rest_read::get_feature_vector_blocking(&self.fv, entry_payload, passed_values, rest_read_options, multithreaded)?;
        tracing::info!("{:?}", sfv);
        Ok(())
    }
}
...
fn entries_to_payload(entry: &Bound<PyAny>) -> PyResult<indexmap::IndexMap<String, serde_json::Value>> {
    let dict = entry.downcast::<PyDict>()?;
    let mut payload: IndexMap<String, serde_json::Value> = indexmap::IndexMap::with_capacity(dict.len());
    for (k, v) in dict {
