...
class FeatureGroup:
    ...
    def __init__(self):
        raise NotImplementedError(
            "Feature Group cannot be instantiated via init method."
        )
    ...
    def read_from_offline_store(
        ...
        if return_type.lower() == "polars":
            ...
            raise NotImplementedError(
                """Supported return type are `"polars"` and `"pyarrow`"."""
            )
    ...
    def read_from_online_store(
        self,
        return_type: Literal["polars", "pyarrow"] = "polars",
        client: Literal["sql", "rest"] = "sql",
    ) -> Union[pl.DataFrame, pa.RecordBatch]:
        if client.lower() == "rest":
            raise NotImplementedError(
                "Rest client is not implemented for large read_operations."
            )
    ...
    def insert(self, dataframe: pl.DataFrame) -> job_execution.JobExecution:
        return job_execution.JobExecution._from_pyjobexec(
