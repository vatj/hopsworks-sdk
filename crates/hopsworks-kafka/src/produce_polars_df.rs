...
// Can segregate serialization and producing into separate functions to run in separate threads
// however, this adds boilerplate and an extra channel/queue to pass the serialized values instead 
// of enqueuing directly into the producer.
// Use separate join_set and create channel in main function to pass serialized values to producer threads
// let (trx, recx) = tokio::sync::mpsc::channel::<(String,Vec<u8>)>(1000000);
#[warn(dead_code)]
async fn send_via_kafka(
    idx: usize,
    producer_config: ClientConfig,
    topic_name: Arc<String>,
    headers: rdkafka::message::OwnedHeaders,
    mut recx: tokio::sync::mpsc::Receiver<(String, Vec<u8>)>,
) -> Result<usize> {
...
fn serialize_chunk(
    idx: usize,
    record: Arc<Record>,
    trx: tokio::sync::mpsc::Sender<(String, Vec<u8>)>,
    rx: tokio::sync::oneshot::Receiver<DataFrame>,
) -> Result<usize> {
    tracing::debug!("Processing chunk: {}", idx);
    let frame: DataFrame = rx.blocking_recv()?;
