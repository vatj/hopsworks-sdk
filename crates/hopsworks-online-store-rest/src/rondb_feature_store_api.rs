...
pub async fn get_single_feature_vector(payload: SingleEntryPayload) -> Result<SingleFeatureVector> {
    let resp = get_online_store_rest_client()?
        .request(Method::POST, "feature_store", true)
        .await?
        .json(&payload)
        .send()
        .await?;
...
pub async fn get_batch_feature_vectors(
    payload: BatchEntriesPayload,
) -> Result<BatchFeatureVectors> {
    let resp = get_online_store_rest_client()?
        .request(Method::POST, "batch_feature_vector", true)
        .await?
        .json(&payload)
        .send()
        .await?;
